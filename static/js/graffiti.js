(() => {
    ...
    const FN = {
        ...
        window_event: () => {
            window.scroll_fn['graffiti'] = (force) => {

                if (!force && (!document.querySelector('#graffiti')
                    || window.scroll_fn['graffiti'].locked
                    || status.innerText === 'end')) return;
    ...
    (() => {//控制相关
        ...
        controller.querySelector('[data-action="submit"]').addEventListener('click', () => {
            Util.ajax({
                ...
                callback: (d) => {
                    Waterfall.reset();
                    window.scroll_fn['graffiti'](true);
                }
            })
        });
